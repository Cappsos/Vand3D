...
from utils.io_lung import load_gt_mask, load_brain_volume
...
def validation_dice3d(args):
    ...
    dice = np.zeros(len(thresholds), dtype=np.float64)
    ...
    with open(results_file, 'w') as f:
        ...
        f.write(f"Number of test samples: {n_patients}\n")
        f.write(f"Best threshold: {best_thr:.3f}\n")
        f.write(f"Best Dice3D: {best_dice:.4f}\n")
        f.write(f"\nThreshold vs Dice curve:\n")
        for t, d in zip(thresholds, dice):
            f.write(f"{t:.3f}  {d:.4f}\n")

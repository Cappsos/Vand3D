...
from models.decoder import FiLMUNet3D
import random
import logging
...
from utils.io_lung import load_gt_mask
...
def train(args):
    ...
    decoder = FiLMUNet3D(
        ...
        in_token_dim=output_adapter_dim,  # same dim as LinearAdapter output
        t_dim=output_adapter_dim,
        base_c=64,
        deep_supervision=False,
        voxel_size=(32, 240, 240) 
    ).to(device)
    ...
    optimizer = torch.optim.Adam(list(trainable_layer.parameters())+ list(decoder.parameters()), lr=learning_rate, betas=(0.5, 0.999))
    ...
    for epoch in range(epochs):
        ...
        for items in tqdm(train_dataloader):
            ...
            with torch.cuda.amp.autocast():
                # 1) Encode image — no grads
                with torch.no_grad():
                    _, _, vit_layers = model.encode_image(volume, features_list)
                
            

                # 2) Patch selection & adaptation
                selected_tokens = [lay[:, 1:, :] for lay in vit_layers]          # drop CLS
                adapted_tokens  = trainable_layer(selected_tokens)               # list[L] (B,P,768)

                # 3) Text embedding (same for the batch)
                text_embed = stacked_text_features.mean(dim=1).to(device).unsqueeze(0).repeat(volume.size(0), 1)                  # (B,768)

                # 4) Grid shape for reshaping tokens
                grid_shape = (
                    model.config.img_size[0] // model.config.patch_size[0],      # D_p
                    model.config.img_size[1] // model.config.patch_size[1],      # H_p
                    model.config.img_size[2] // model.config.patch_size[2],      # W_p
                )

                # 5) Decoder
                pred_logit= decoder(adapted_tokens, text_embed, grid_shape)
                #print(f"DEBUG DECODER OUTPUTS:")
                #print(f"  grid_shape: {grid_shape}")
                #print(f"  pred_logit shape: {pred_logit.shape}")
                #print(f"  Expected: (B, 1, D, H, W) where D,H,W could be patch or full resolution")
               
               
                prob_fg    = torch.sigmoid(pred_logit)*3      # foreground prob


            gt = items['img_mask'].float().to(device)   # (B,D,H,W)
            gt[gt > .5] = 1; gt[gt <= .5] = 0
            

            prob_full = torch.cat([1 - prob_fg, prob_fg], dim=1)      # (B,2,D,H,W)

            # focal uses 2-ch probs + 1-ch labels
            
            loss  = loss_focal(prob_full, gt.unsqueeze(1))                       # FocalLoss
            # dice uses foreground prob vs. scalar mask
            loss += loss_dice(prob_fg.squeeze(1), gt)                 # BinaryDice
            ...
            for probs, logits in zip(layer_anomaly_maps_full_res, upsampled_logits_list):
                ...
                with torch.no_grad():
                    ...
                    # 2) Log the modulating factor
                    mod = (1 - p_t).pow(loss_focal.gamma)
                    #print(f"Modulator — mean {mod.mean():.3f}, std {mod.std():.3f}")

                    # 3) Log the underlying cross‐entropy term 
                    ce = -torch.log(p_t.clamp(1e-6,1-1e-6))
                    #print(f"CE term — mean {ce.mean():.3f}, std {ce.std():.3f}")
                    #fl_map = (loss_focal.alpha * mod * ce)
                    #print(f"FL map — mean {fl_map.mean():.3f}, std {fl_map.std():.3f}")

                pred_fg = probs[:, 1:2, ...]  # [B,1,D,H,W]
                
                loss += loss_focal(logits, gt)
        ...
        val_metrics = validation(model, trainable_layer, decoder, val_dataloader, stacked_text_features, device, args, save_path, adapter_k, scales=args.fusion_scales)
        ...
        epoch_data = {
            ...
            #'train_dice_loss': avg_ if aux_half is not None:
            **val_metrics
        }
...
def validation(model, linearlayer, decoder, val_dataloader, stacked_text_features, device, args, save_path, adapter_k=3, scales=[1]):
    """
    ...
    # Add decoder initialization for validation
    feature_dim = model.config.hidden_size
    output_adapter_dim = model.config.hidden_size
    ...
    for idx, items in enumerate(tqdm(val_dataloader, desc="Processing sub-volumes")):
        ...
        full_shape = (32, 240, 240)               # D, H, W  (your upsampled size)
        ...
        with torch.no_grad(), torch.cuda.amp.autocast():
            # 1) Encode image — no grads
            _, _, vit_layers = model.encode_image(volume, args.features_list)
            
            # 2) Patch selection & adaptation
            selected_tokens = [lay[:, 1:, :] for lay in vit_layers]          # drop CLS
            adapted_tokens  = linearlayer(selected_tokens)               # list[L] (B,P,768)
            ...
            # Extract anomaly map from decoder output
            if args.depth == 1:
                # If depth is 1, pick only central slice
                final_anomaly_map = prob_fg[:, 0, 16, :, :].squeeze(0).cpu().numpy()  # [H,W]
            else:
                final_anomaly_map = prob_fg.squeeze(0).squeeze(0).cpu().numpy()  # [D,H,W]
                
        # ...existing code for fusion and saving...
        y0, x0, h_crop, w_crop = items['crop_box']
        
        # For decoder output, we only have one final map instead of multiple layers
        layer_np = final_anomaly_map
        ...
        if args.depth == 1:
            ...
            # For 3D case
            tmp = torch.from_numpy(layer_np)[None, None].float()
            resized = torch.nn.functional.interpolate(
                tmp,
        ...
        # Store for each "layer" (though decoder gives us one output, we can replicate for compatibility)
        for i in range(adapter_k):
    ...
    for patient_dir in patient_dirs:
        ...
        try:
            ...
            gt_path = os.path.join(gt_root, f'{patient_id}/{patient_id}-seg.npy')
            gt = np.load(gt_path)
    ...
    thresholds = np.linspace(0, 1, 21)  # Reduced from 51 to 21 for faster validation
    ...
    for pred, gt in zip(all_preds, all_gts):
        ...
        gt = (gt > 0).astype(np.uint8)  # Ensure GT is binary

...
class BraTS3DSubVolumeDataset(data.Dataset):
    def __init__(
        ...
        for patient_info in patient_volume_infos:
            ...
            sample_volume = np.load(volume_path)
            
            original_depth = sample_volume.shape[2]
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            # Extract the 32-slice sub-volume along depth
            
            
            if start_d + self.sub_volume_depth > d:
                print(f"Warning: Requested slice {start_d}:{start_d + self.sub_volume_depth} exceeds volume depth {d}")
                # This shouldn't happen with proper block_starts calculation, but safety check
                start_d = max(0, d - self.sub_volume_depth)
        ...
        if anomaly_label == 0 or full_mask_path_rel is None:
            ...
            if not full_mask_path.exists():
                ...
                try:
                    ...
                    mh, mw, md = full_mask_np_hwd.shape
                    if start_d + self.sub_volume_depth > md:
                        start_d_mask = max(0, md - self.sub_volume_depth)
                    else:
                        start_d_mask = start_d
                  
                    sub_mask_np_hwd = full_mask_np_hwd[:, :, start_d_mask : start_d_mask + self.sub_volume_depth]

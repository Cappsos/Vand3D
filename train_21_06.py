...
def train(args):
    ...
    if args.patient_seed_path is not None and os.path.exists(args.patient_seed_path):
        train_data = BraTS3DSubVolumeDataset(root=args.train_data_path, transform_img=img_transform, transform_mask=mask_transform, dataset_name=args.dataset, mode ='train', stride_depth=stride, sub_volume_depth=args.depth, patient_seed_path=args.patient_seed_path)
    else:
        train_data = BraTS3DSubVolumeDataset(root=args.train_data_path, transform_img=img_transform, transform_mask=mask_transform, dataset_name=args.dataset, mode ='train', stride_depth=stride, sub_volume_depth=args.depth)
    ...
    for epoch in range(epochs):
        ...
        for items in tqdm(train_dataloader):
            ...
            for probs in layer_anomaly_maps_full_res:
        ...
        val_metrics = validation(model, trainable_layer, val_dataloader, stacked_text_features, device, args, save_path, adapter_k)
...
def validation(model, linearlayer, val_dataloader, stacked_text_features, device, args, save_path, adapter_k=3):
    """

...
def train(args):
    ...
    mask_transform = Transform3DMask(target_height=256, target_width=256, target_depth=args.depth)
    ...
    if args.patient_seed_path is not None and os.path.exists(args.patient_seed_path):
        train_data = Lung3DSubVolumeDataset(
    ...
    val_data = Lung3DSubVolumeDataset(
    ...
    val_dataloader = torch.utils.data.DataLoader(val_data, batch_size=1, shuffle=False, num_workers=2)
...
def validation(model, linearlayer, val_dataloader, stacked_text_features, device, args, save_path, adapter_k=3, scales=[1]):
    ...
    for idx, items in enumerate(tqdm(val_dataloader, desc="Processing sub-volumes")):
        ...
        full_shape = (32, 512, 512)               # D, H, W  (upsampled size)
    ...
    for patient_dir in patient_dirs:
        ...
        try:
            ...
            # Load full GT mask - use the patient_id without depth suffix
...
def arg_parser_with_config():
    ...
    parser.add_argument("--val_data_path", type=str, default="/home/nicoc/Task06_Lung", help="validation dataset path")
    parser.add_argument("--gt_root", type=str, default="/home/nicoc/Task06_Lung/labelsTr_numpy", help="gt path")

...
def train(args):
    ...
    if args.prompt_ensemble:
     ...
     #print(torch.linalg.norm(stacked_text_features.float(), dim=0))
    # debug cosine similarity text features
    with torch.cuda.amp.autocast(enabled=False):
        stacked_text_features = torch.nn.functional.normalize(
            stacked_text_features.float(), dim=0, eps=1e-6
        )
    ...
    print(torch.__version__, torch.version.cuda, torch.get_autocast_gpu_dtype())
    logger.info("="*50)
        ...
        for items in tqdm(train_dataloader):
            ...
            with torch.cuda.amp.autocast(enabled=False):
                ...
                for i in range(adapter_k):
                   ...
                   # print(adapted_patch_features_this_layer)
                    
                    # Normalize features from this adapted layer
                    adapted_patch_norm = F.normalize(adapted_patch_features_this_layer, dim=-1, eps=1e-6)
                    #print(adapted_patch_norm)
    ...
    for idx, items in enumerate(tqdm(val_dataloader, desc="Processing sub-volumes")):
        ...
        with torch.no_grad(), torch.cuda.amp.autocast():
            ...
            for i in range(adapter_k):
                ...
                adapted_patch_norm = F.normalize(adapted_patch_features, dim=-1, eps=1e-6)

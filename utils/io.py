...
def load_gt_mask(patient_id, gt_root):
    """
    Given a patient_id like 'BraTS-MET-00010-000', find and load
    the single '*-seg.npy' file in gt_root/patient_id/,
    binarize (>0) and return an array of shape (155,240,240).
    """
    pat_dir = os.path.join(gt_root, patient_id)
    seg_files = [f for f in os.listdir(pat_dir) if f.endswith('-seg.npy')]
    if len(seg_files) != 1:
        raise FileNotFoundError(
            f"Expected exactly one '*-seg.npy' in {pat_dir}, found {seg_files}"
        )
    seg = np.load(os.path.join(pat_dir, seg_files[0]))
    
    # Fix: Ensure correct shape (155, 240, 240) - depth first
    if seg.shape != (155, 240, 240):
        #print(f"Warning: GT shape {seg.shape} for {patient_id}, attempting to fix...")
        
        # Common transpose cases
        if seg.shape == (240, 240, 155):
            seg = np.transpose(seg, (2, 0, 1))  # (H,W,D) -> (D,H,W)
        elif seg.shape == (240, 155, 240):
            seg = np.transpose(seg, (1, 0, 2))  # (H,D,W) -> (D,H,W)
        else:
            raise ValueError(f"Unexpected GT shape {seg.shape} for {patient_id}")
